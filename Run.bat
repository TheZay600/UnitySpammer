...
cls
@echo off
IF ERRORLEVEL 1 (GOTO :Next1) ELSE (ECHO "%MyFile%" Unity Spammer was blocked by your Antivirus!)
...
cls
@echo off
IF ERRORLEVEL 1 (GOTO :Next2) ELSE (ECHO "%MyFile%" Unity Spammer was blocked by your Windows Defender Antivirus!)
echo Please Disable your Antivirus to continue.
pause
...
:right
@echo off
start src\application\Unity.exe
cd src
node .
pause
...
:right
@echo off
start src\application\UnitySpammer.exe
cd src
node .
pause